...
!!! important
    Before starting, make sure your **local** `development`, `main`, and (if applicable) the current LTM branch are all up to date with upstream!

    ```
    git fetch
    git switch development && git pull # and repeat for main/ltm
    ```

Choose your own adventure:

- LTM release? Jump [here](#ltm-releases).
- Patch release from `development`? Jump [here](#all-releases-from-development).
- Minor release? Continue with [Minor Version Bumps](#minor-version-bumps) and then [All Releases from `development`](#all-releases-from-development).
...
### Sync the Release Back to `development`

After a release has been published from the `main` branch, a new PR will automatically be created to merge the changes from `main` back into `development` with a version bump to the next development version (e.g. `1.4.3a1`). Review and merge this PR once CI has completed and the PR has been approved.
...
```no-highlight
> poetry version
nautobot-cellular-sot 1.0.0-beta.2
```
...
First, create a release branch off of `development` (`git switch -c release-1.4.2 development`) and automatically generate release notes with `invoke generate-release-notes`.
...
Draft a [new release](https://github.com/Lipford-Dutch/nautobot-wireless-cellular-sot-app/releases/new) with the following parameters.
...
- Change the entries generated by GitHub to only the usernames of the contributors. e.g. `* Updated dockerfile by @nautobot_user in https://github.com/Lipford-Dutch/nautobot-wireless-cellular-sot-app/pull/123` -> `* @nautobot_user`.
...
**Full Changelog**: https://github.com/Lipford-Dutch/nautobot-wireless-cellular-sot-app/compare/v1.4.1...v1.4.2
```

Publish the release!

### Create a PR from `main` back to `development`

First, sync your `main` branch with upstream changes: `git switch main && git pull`.

Create a new branch from `main` called `release-1.4.2-to-development` and use `poetry version prepatch` to bump the development version to the next release.

For example, if you just released `v1.4.2`:

```no-highlight
> git switch -c release-1.4.2-to-development main
Switched to a new branch 'release-1.4.2-to-development'
...
!!! important
    Do not squash merge this branch into `development`. Make sure to select `Create a merge commit` when merging in GitHub.

Open a new PR from `release-1.4.2-to-development` against `development`, wait for CI to pass, and merge it.
...
The process is similar to [releasing from `development`](#all-releases-from-development), but there is no need for post-release branch syncing because you'll release directly from the LTM branch.

Once the release has been published, open a separate PR against `development` to synchronize all LTM release notes into the latest version of the docs for visibility.
...
9. Open a separate PR against `development` to synchronize all LTM release notes into the latest version of the docs for visibility.

You can use the following commands to help you pull in the release notes from the `ltm-2.4` branch to the `development` branch:
...
> git switch -c release-2.4.99-notes-to-development development

> git checkout ltm-2.4 docs/admin/release_notes/version_2.4.md

> git add docs/admin/release_notes/version_2.4.md && git commit -m "Update release notes from LTM 2.4 to development"

> git push
```

Open a new PR from `release-2.4.99-notes-to-development` against `development`, wait for CI to pass, and merge it.
